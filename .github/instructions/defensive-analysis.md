...
**: any` as a validation gap signal:** When `: any` annotations appear in server route files, check whether the surrounding code has proper schema validation. If not, report it as both a type safety issue (Check 3) and a validation gap (Check 5).
